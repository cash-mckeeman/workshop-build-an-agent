...
from src.examples.simple_chat import simple_chat_demo
...
class TestSimpleChatDemo:
    ...
    @patch("src.examples.simple_chat.create_basic_agent")
    @patch("src.examples.simple_chat.load_dotenv")
    @patch("builtins.print")
    def test_simple_chat_demo_success(
        ...
        mock_create_basic_agent.assert_called_once_with(
            model_name="microsoft/DialoGPT-medium", temperature=0.8, max_new_tokens=256
        )
        ...
        assert any(
            "✅ Simple chat demo completed!" in str(call) for call in print_calls
        )

    @patch("src.examples.simple_chat.create_basic_agent")
    @patch("src.examples.simple_chat.load_dotenv")
    @patch("builtins.print")
    def test_simple_chat_demo_agent_creation_error(
        self, mock_print, mock_load_dotenv, mock_create_basic_agent
    ):
        """Test simple chat demo when agent creation fails."""
        mock_create_basic_agent.side_effect = Exception("Failed to create agent")
    ...
    @patch("src.examples.simple_chat.load_dotenv")
    @patch("builtins.print")
    def test_simple_chat_demo_chat_error(
        self, mock_print, mock_load_dotenv, mock_create_basic_agent
    ):
    ...
    @patch("src.examples.simple_chat.load_dotenv")
    @patch("builtins.print")
    def test_simple_chat_demo_partial_responses(
        self, mock_print, mock_load_dotenv, mock_create_basic_agent
    ):
    ...
    @patch("src.examples.simple_chat.load_dotenv")
    @patch("builtins.print")
    def test_simple_chat_demo_output_format(
        self, mock_print, mock_load_dotenv, mock_create_basic_agent
    ):
...
class TestSimpleChatModuleIntegration:
    ...
    def test_dotenv_path_resolution(self, mock_load_dotenv):
        ...
        with patch("src.examples.simple_chat.create_basic_agent") as mock_create_agent:
            mock_agent = Mock()
...
class TestMainExecution:
    ...
    @patch("src.examples.simple_chat.simple_chat_demo")
