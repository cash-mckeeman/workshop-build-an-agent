...
def create_tool_wrapper(func: Callable) -> Callable:
    ...
    Example:
        from tools.demo_tools import word_count
        from tools.agent_helpers import create_tool_wrapper
...
def make_agent_tools(*functions):
    ...
    Example:
        from tools.demo_tools import word_count, reverse_string
...
class ToolCallInfo:
    ...
    call_part_index: Optional[int] = None
    return_part_index: Optional[int] = None
...
def extract_tool_calls_from_messages(messages: List[Any], debug: bool = False) -> List[ToolCallInfo]:
    ...
    for i, msg in enumerate(messages):
        ...
        try:
            ...
            if hasattr(msg, 'parts') and msg.parts:
                for part_idx, part in enumerate(msg.parts):
                    ...
                    if hasattr(part, 'tool_name') and hasattr(part, 'tool_call_id') and not hasattr(part, 'content'):
                        ...
                        if debug:
                            print(f"  ✓ Found ToolCallPart: {tool_info.tool_name} (call_id: {call_id})")
                    ...
                    # Check for ToolReturnPart
                        ...
                        if call_id and call_id in call_parts:
                            ...
                            if debug:
                                print(f"  ✓ Matched ToolReturnPart to existing call: {tool_name} -> {result[:50]}...")
                        else:
                            ...
                            tool_calls.append(tool_info)
                            if debug:
                                print(f"  ✓ Found orphaned ToolReturnPart: {tool_name} -> {result[:50]}...")
                ...
                # Check for tool_calls in assistant messages
                    ...
                    for j, tool_call in enumerate(msg.get('tool_calls', [])):
                        if isinstance(tool_call, dict):
                            ...
                            tool_calls.append(tool_info)
                            if debug:
                                print(f"  ✓ Found assistant tool call {j}: {tool_info.tool_name}")
            ...
            if hasattr(msg, 'tool_calls') and msg.tool_calls:
                for tool_call in msg.tool_calls:
                    tool_info = ToolCallInfo(
                        ...
                        arguments=getattr(tool_call, 'arguments', None) or getattr(tool_call, 'function', {}).get('arguments'),
                        call_id=getattr(tool_call, 'id', None),
                        message_index=i
                    )
                    tool_calls.append(tool_info)
            ...
            if hasattr(msg, 'function_call') or (isinstance(msg, dict) and 'function_call' in msg):
                func_call = msg.function_call if hasattr(msg, 'function_call') else msg['function_call']
                tool_info = ToolCallInfo(
                    tool_name=func_call.get('name') if isinstance(func_call, dict) else str(func_call),
                    arguments=func_call.get('arguments') if isinstance(func_call, dict) else None,
                    message_index=i
                )
                tool_calls.append(tool_info)
                if debug:
            ...
            if hasattr(msg, 'content') and isinstance(msg.content, str):
                ...
                for pattern, indicator in tool_patterns:
                    ...
                    for match in matches:
                        ...
                        tool_calls.append(tool_info)
                        if debug:
                            print(f"  ✓ Inferred tool from content: {tool_info.tool_name}")
    ...
    if debug:
        ...
        for tc in tool_calls:
            status = "✓ Complete" if (tc.has_matching_return or tc.result) else "⚠ Call only"
...
def analyze_agent_execution(result, agent_type: str, question: str) -> AgentDiagnostics:
    ...
    try:
        ...
        confidence_indicators = _analyze_response_confidence(result.output, agent_type, question, tool_calls)

        # Count only complete tool executions for diagnostics
        complete_executions = [tc for tc in tool_calls if tc.has_matching_return or tc.result]
        has_tools_executed = len(complete_executions) > 0

        # Generate execution summary
        summary = _generate_execution_summary(agent_type, question, tool_calls, confidence_indicators)
...
def _analyze_response_confidence(output: str, agent_type: str, question: str, tool_calls: List[ToolCallInfo]) -> Dict[str, Any]:
    ...
    numeric_indicators = [r'=\s*\d+', r'equals?\s+\d+', r'result:\s*\d+', r'answer:\s*\d+']
    ...
    for category, keywords in tool_keywords.items():
        if any(keyword in question_lower for keyword in keywords):
            ...
            if category == 'time' and 'time' in agent_tools:
                ...
                break
            elif category == 'math' and any(math_tool in agent_tools for math_tool in ['add', 'multiply']):
                indicators['expected_tool_usage'] = True
                indicators['expected_tool_category'] = category
                indicators['agent_has_required_tools'] = True
                break
    ...
    if indicators['expected_tool_usage'] and indicators['agent_has_required_tools'] and not indicators['tools_executed']:
        # The agent should have tools and should have used them
        if indicators.get('expected_tool_category') == 'time' and indicators.get('data_type') == 'time':
            ...
            indicators['confidence_note'] = 'Response contains time data - tools may have been executed but not detected'
        else:
            indicators['potential_hallucination'] = True
            indicators['confidence_note'] = 'Expected tool usage but no tools executed - response may be hallucinated'
    elif indicators['expected_tool_usage'] and not indicators['agent_has_required_tools']:
        # Question requires tools this agent doesn't have - this is expected behavior
        indicators['potential_hallucination'] = False
        indicators['confidence_note'] = f'{agent_type.title()} agent does not have required tools for this question'
    elif not indicators['expected_tool_usage']:
        # Question doesn't require tools - text-only response is appropriate
        indicators['potential_hallucination'] = False
        indicators['confidence_note'] = 'Text-only response appropriate for this question'

    return indicators


def _generate_execution_summary(agent_type: str, question: str, tool_calls: List[ToolCallInfo], confidence: Dict[str, Any]) -> str:
    """Generate a human-readable execution summary."""
    if not tool_calls:
        if confidence.get('potential_hallucination'):
            return f"⚠️ No tools executed - response may be hallucinated"
...
def should_expect_tools(agent_type: str, question: str) -> bool:
    ...
    if agent_type == 'math' and any(keyword in question_lower for keyword in math_keywords):
        return True

    # Demo/utility keywords
    utility_keywords = ['time', 'fact', 'word', 'reverse', 'palindrome', 'count']
    if agent_type in ['demo', 'tools'] and any(keyword in question_lower for keyword in utility_keywords):
        return True
